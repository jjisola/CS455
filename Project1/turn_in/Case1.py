...
logPositions_x = np.zeros((nodes, iterations), dtype=np.float16)
logPositions_y = np.zeros((nodes, iterations), dtype=np.float16)

logVelocitity_x = np.zeros((nodes, iterations), dtype=np.float16)
logVelocitity_y = np.zeros((nodes, iterations), dtype=np.float16)

logVelocitityMagnitude = np.zeros((nodes, iterations), dtype=np.float16)

logConnectivity = np.zeros((iterations), dtype=np.float16)
...
def u_i_alpha(node):
    gradientSum = np.array([0, 0], dtype=np.float16)
    consensusSum = np.array([0, 0], dtype=np.float16)
    ...
    for neighbor in neighbors:
        ...
        diff_x =np.array([velocity_x[neighbor] - velocity_x[node], velocity_y[neighbor] - velocity_y[node]], dtype=np.float16)
    ...
    for i in range(0, iterations):
        ...
        #case: each next iteration -> update velocity, position
            ...
            for j in range(nodes):
                ...
                alphaAgent = u_i_alpha(j)
                tmpVelocity = [velocity_x[j], velocity_y[j]]
                tmpPosition = [x_coordinates[j], y_coordinates[j]]

                v = tmpVelocity + (alphaAgent * delta_t)
                p = tmpPosition + (v * delta_t) + ((1/2) * (delta_t ** 2) * alphaAgent)
